...
(function initThree(){
  ...
  // Wireframe sphere — cyber globe
  const sGeo = new THREE.IcosahedronGeometry(2.2,3);
  const sMat = new THREE.MeshBasicMaterial({
    color:0x00ff9f,wireframe:true,transparent:true,opacity:.06
  });
...
/* ── RUST/WASM PARTICLE PHYSICS ENGINE (SIMULATED WASM) ─── */
/*
  In a real Rust/WASM build, this module would be compiled from:
  ...
  We simulate the exact same behavior in JS,
  exposing the identical API surface as if loaded via:
  import init, { ParticleEngine } from './mad_wasm.wasm';
*/
const WasmParticleEngine = (function(){
  const STRUCT_SIZE = 8; // x,y,vx,vy,life,maxLife,size,alpha
  ...
  ParticleEngine.prototype._initParticle = function(i){
    ...
    b[o]   = Math.random()*this._w;       // x
    b[o+1] = Math.random()*this._h;       // y
    b[o+2] = (Math.random()-.5)*.4;       // vx
    b[o+3] = (Math.random()-.5)*.4 - .1;  // vy
    b[o+4] = 0;                            // life
    b[o+5] = 120+Math.random()*180;        // maxLife
    b[o+6] = .5+Math.random()*1.5;        // size
    b[o+7] = 0;                            // alpha
  };

  // Tick — equivalent to Rust's impl ParticleEngine { pub fn tick }
  ParticleEngine.prototype.tick = function(dt, mx, my){
    const b = this._buf;
    const W = this._w, H = this._h;
    for(let i=0;i<this._count;i++){
      const o = i*STRUCT_SIZE;
      // Rust-equivalent: particle physics update
      let x=b[o],y=b[o+1],vx=b[o+2],vy=b[o+3],life=b[o+4],maxLife=b[o+5],sz=b[o+6];

      // Mouse repulsion field
      const dx=x-mx, dy=y-my;
      const dist=Math.sqrt(dx*dx+dy*dy);
      if(dist<120&&dist>0){
        const force=(.6/dist)*dt*60;
      ...
      let alpha;
      if(t<.15) alpha=t/.15;
      else if(t>.7) alpha=(1-t)/.3;
      else alpha=1;
      alpha*=.55;

      if(life>=maxLife||x<-10||x>W+10||y<-10||y>H+10){
        this._initParticle(i);
      } else {
        b[o]=x;b[o+1]=y;b[o+2]=vx;b[o+3]=vy;b[o+4]=life;b[o+7]=alpha;
      }
    }
  };
...
(function initCanvas(){
  ...
  let mx=W/2,my=H/2;
  let lastT=0, fps=0, fpsCounter=0, fpsTimer=0;
  ...
  function render(ts){
    ...
    if(fpsTimer>=.5){fps=Math.round(fpsCounter/fpsTimer);fpsCounter=0;fpsTimer=0;
      document.getElementById('wasm-counter').textContent=
        `particles: ${engine.count()} | fps: ${fps}`;
    }
    ...
    for(let i=0;i<engine.count();i++){
      const o=i*8;
      const x=buf[o],y=buf[o+1],sz=buf[o+6],alpha=buf[o+7];
    ...
    for(let i=0;i<engine.count();i+=2){
      for(let j=i+1;j<engine.count();j+=2){
        const oi=i*8,oj=j*8;
        const dx=buf[oi]-buf[oj],dy=buf[oi+1]-buf[oj+1];
        const d2=dx*dx+dy*dy;
        if(d2<6400){ // 80px
          const a=(1-d2/6400)*.08*Math.min(buf[oi+7],buf[oj+7])*2;
          ctx.strokeStyle=`rgba(0,255,159,${a})`;
          ctx.beginPath();ctx.moveTo(buf[oi],buf[oi+1]);ctx.lineTo(buf[oj],buf[oj+1]);ctx.stroke();
        }
      }
    }
  }
...
(function initCursor(){
  ...
  function loop(){
    rx+=(cx-rx-12)*.15;ry+=(cy-ry-12)*.15;
    cross.style.cssText=`left:${Math.round(rx)}px;top:${Math.round(ry)}px`;
...
(function clock(){
  ...
  function upd(){
    const d=new Date();
    el.innerHTML=`${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}:${String(d.getSeconds()).padStart(2,'0')}<span class="blink" style="font-family:var(--mono)">_</span>`;
  }
    ...
    entries.forEach(e=>{
      if(e.isIntersecting){
        e.target.querySelectorAll('.sk-fill').forEach(f=>{
          f.style.width=(parseFloat(f.dataset.v)*100)+'%';
          f.classList.add('on');
        });
...
(function manifesto(){
  const words=[
    ...
    {t:"Cisco,",c:"pur"},{t:"chaque",c:""},{t:"certification",c:"crt"},
  ...
  const obs=new IntersectionObserver(()=>{
    spans.forEach((s,i)=>{
      setTimeout(()=>s.classList.add('lit'),i*55);
    });
    obs.disconnect();
  },{threshold:.3});
  obs.observe(el);
